...
def generate_basis(rank, seed, max_in_out=None, state_dict=None, shared=True, sparse=False):
    basis = {}
...
class VeRAMergedLinear(nn.Linear, LoRALayer):
    ...
    def reset_parameters(self):
        ...
        if hasattr(self, 'randlora_gamma'):
            self.randlora_lambda.data = torch.ones(self.randlora_lambda.shape) / 10
            self.randlora_gamma.data = torch.zeros(self.randlora_gamma.shape)
    ...
    def merge_basis(self, merged=True):
...
class ConvVeRA(nn.Module, LoRALayer):
    def __init__(self, in_features, out_features, r=0, lora_alpha=1, lora_dropout=0., merge_weights=True, basis = None, **kwargs):
        ...
        if r > 0:
            ...
            self.randlora_gamma  = nn.Parameter(self.weight.new_zeros((self.n, in_features)))
    ...
    def merge_basis(self):
        b, a = self.randlora_lambda, self.randlora_gamma
        lora_a = LoadParams.apply(self.basis_a[:self.n, :, :self.in_features], b, a)
        
        lora_b = self.basis_b[:self.n, :self.out_features, :]
        lora_b, lora_a = lora_b.permute(1,0,2).flatten(start_dim=1).to(lora_a), lora_a.permute(1,0,2).flatten(end_dim=1)
    ...
    def train(self, mode=True):
        super(ConvVeRA, self).train(mode)
        if mode:
            if self.merge_weights and self.merged:
                if self.r > 0:
                    lora_B, lora_A = self.merge_basis()

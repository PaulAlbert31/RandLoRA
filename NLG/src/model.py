...
class Attention(nn.Module):
    def __init__(self, nx, n_ctx, config, scale=False, basis=None):
        ...
        self.scale = scale
        if config.randlora:
            self.c_attn = lora.RandLoRAMergedLinear(
                nx, n_state * 3, 
            ...
            self.c_attn = lora.VeRAMergedLinear(
                nx, n_state * 3, 
                r=config.lora_attn_dim, 
                lora_alpha=config.lora_attn_alpha, 
                lora_dropout=config.lora_dropout, 
...
class MLP(nn.Module):
    def __init__(self, n_state, config, basis=None):  # in MLP: n_state=3072 (4 * n_embd)
        super(MLP, self).__init__()
        nx = config.n_embd
        self.basis = basis
        if config.randlora:
            self.c_fc = lora.ConvRandLoRA(
                n_state,
                nx,
                r=config.lora_attn_dim, 
                lora_alpha=config.lora_attn_alpha, 
...
class Block(nn.Module):
    def __init__(self, n_ctx, config, scale=False, basis=None):
        super(Block, self).__init__()
        nx = config.n_embd
        self.ln_1 = LayerNorm(nx, eps=config.layer_norm_epsilon)
        self.attn = Attention(nx, n_ctx, config, scale, basis=basis)
        self.ln_2 = LayerNorm(nx, eps=config.layer_norm_epsilon)
        self.mlp = MLP(4 * nx, config, basis=basis)
...
class GPT2Model(nn.Module):
    def __init__(self, config):
        ...
        self.config = config        
        if self.config.randlora:
            self.basis = lora.generate_basis(self.config.lora_attn_dim, self.config.seed, max_in_out=(config.n_ctx*4, config.n_ctx))
        elif self.config.vera:
            self.basis = lora.generate_basis(self.config.lora_attn_dim, self.config.seed, n=1, max_in_out=(config.n_ctx*4, config.n_ctx*4))
        else:
            self.basis = None    
            
        self.wte = nn.Embedding(config.vocab_size, config.n_embd)
        self.wpe = nn.Embedding(config.n_positions, config.n_embd)
        block = Block(config.n_ctx, config, scale=True, basis=self.basis)

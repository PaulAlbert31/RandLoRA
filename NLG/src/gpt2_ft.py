...
parser.add_argument('--randlora', default=False, action='store_true', help='Use RandLoRA in place of LoRA')
parser.add_argument('--vera', default=False, action='store_true', help='Use VeRA in place of LoRA')
parser.add_argument('--seed', default=1, type=int, help='Seed for RandLoRA/VeRA random bases')
...
def optimizer_step(_loss, _optimizer, _model, _schedule, args, is_update=True):
    if args.fp16:
       args.scaler.scale(_loss).backward()
    else:
        _loss.backward()
...
if __name__ == '__main__':
    ...
    if args.fp16:
        args.scaler = torch.amp.GradScaler()        
    ...
    if args.randlora:
        config.randlora = args.randlora
        config.seed = int(torch.exp(torch.tensor(args.random_seed))*3.1415)
    elif args.vera:
        config.vera = args.vera
        config.seed = int(torch.exp(torch.tensor(args.random_seed))*3.1415)
    ...
    num_trainable = sum([p.numel() for p in lm_net.parameters() if p.requires_grad])
    num_params = sum([p.numel() for p in lm_net.parameters()]) - num_trainable

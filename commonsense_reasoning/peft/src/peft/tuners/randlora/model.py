...
class RandLoraModel(BaseTuner):
    ...
    **Attributes**:
    ...
    def _find_dim(self, config) -> tuple[int, int]:
        ...
        This will be used for determining the size of the shared randbasis_A and randbasis_B matrices.
        """
    ...
    def _init_randbasis_A_randbasis_B_sparse(self, config: RandLoraConfig, adapter_name: str, s: int = 3) -> None:
        """
        ...
        # use of persistent to exclude randbasis_A and randbasis_B from the state dict if we choose not to save them.
        self.randbasis_A = BufferDict({}, persistent=config.save_projection)
        self.randbasis_B = BufferDict({}, persistent=config.save_projection)

        # deterministic init of randbasis_A and randbasis_B if we know the key
        generator = torch.Generator(device="cpu").manual_seed(config.projection_prng_key)
        randbasis_A = torch.rand((config.r, 1, linear_out_dim), generator=generator) #The gamma matrix is applied on A meaning it can be unique (shared) accross the n scaling matrices
        
        #Ensure full rank
        n = min(linear_out_dim, linear_in_dim) / config.r
        n = int(n) if n.is_integer() else int(n) + 1 #Ensure full rank
        randbasis_B = torch.rand((linear_in_dim, n, config.r), generator=generator)
        ...
        randbasis_B_sparse = torch.zeros(randbasis_B.shape)
        randbasis_A_sparse = torch.zeros(randbasis_A.shape)
        randbasis_B_sparse[randbasis_B<1/(2*s)] = -1
        randbasis_B_sparse[randbasis_B>1-1/(2*s)] = 1        
        randbasis_A_sparse[randbasis_A<1/(2*s)] = -1
        randbasis_A_sparse[randbasis_A>1-1/(2*s)] = 1
        
        #Std normalization is empirically found to be the best
        randbasis_A, randbasis_B = randbasis_A_sparse / randbasis_A_sparse.std(), randbasis_B_sparse / randbasis_B_sparse.std()
        self.randbasis_A[adapter_name] = randbasis_A
        self.randbasis_B[adapter_name] = randbasis_B
        
    def _init_randbasis_A_randbasis_B(self, config: RandLoraConfig, adapter_name: str) -> None:
        linear_out_dim, linear_in_dim = self._find_dim(config)
        # use of persistent to exclude randbasis_A and randbasis_B from the state dict if we choose not to save them.
        self.randbasis_A = BufferDict({}, persistent=config.save_projection)
        self.randbasis_B = BufferDict({}, persistent=config.save_projection)
        ...
        randbasis_A = _kaiming_init((config.r, 1, linear_out_dim), generator=generator)
        
        #Ensure full rank
        n = min(linear_out_dim, linear_in_dim) / config.r
        n = int(n) if n.is_integer() else int(n) + 1
        randbasis_B = torch.cat([_kaiming_init((linear_in_dim, 1, config.r), generator=generator) for _ in range(n)], dim=1)
        
        #Std normalization is empirically found to be the best
        randbasis_A, randbasis_B = randbasis_A / randbasis_A.std(), randbasis_B / randbasis_B.std()
        self.randbasis_A[adapter_name] = randbasis_A
        self.randbasis_B[adapter_name] = randbasis_B
    ...
    def _pre_injection_hook(self, model: nn.Module, config: RandLoraxConfig, adapter_name: str) -> None:
        if config.very_sparse:
            linear_out_dim, linear_in_dim = self._find_dim(config)
            self._init_randbasis_A_randbasis_B_sparse(config, adapter_name, s=math.sqrt(min(linear_out_dim, linear_in_dim)))
        elif config.sparse:
            self._init_randbasis_A_randbasis_B_sparse(config, adapter_name, s=3)
        else:
            self._init_randbasis_A_randbasis_B(config, adapter_name)
    ...
    @staticmethod
    def _create_new_module(randlora_config, randbasis_A, randbasis_B, adapter_name, target, **kwargs):
        ...
        if loaded_in_8bit and isinstance(target_base_layer, bnb.nn.Linear8bitLt):
            ...
            return Linear8bitLt(target, adapter_name, randbasis_A, randbasis_B, **eightbit_kwargs)
            ...
            return Linear4bit(target, adapter_name, randbasis_A, randbasis_B, **fourbit_kwargs)

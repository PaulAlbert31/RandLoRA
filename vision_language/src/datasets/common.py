...
def maybe_dictionarize(batch, index=False):
    if isinstance(batch, dict):
        if len(batch["images"]) == 2: #asymetric augs
            batch["images_aug"] = batch["images"][1]
    ...
    if len(batch) == 2:
        if isinstance(batch[1], list):#mscoco
            batch[1] = [b["caption"] for b in batch[1]]
...
def get_dataloader(dataset, is_train, args, image_encoder=None):
    ...
    if args.subsample is not None and is_train:
        ...
        new_dataset, _ = torch.utils.data.random_split(src, lengths, gen)

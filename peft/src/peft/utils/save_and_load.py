...
def set_peft_model_state_dict(
    ...
    if config.peft_type in (
        ...
        if config.peft_type == PeftType.ADALORA:
            ...
            if config.save_projection and f"base_model.randbasis_A" not in peft_model_state_dict:#adapter name should not be needed, why is it ?
                raise ValueError(
                    "Specified to load randbasis_A and randbasis_B from state dictionary however they were not present!"
                )
            elif not config.save_projection and f"base_model.randbasis_A" in peft_model_state_dict:
                warnings.warn(
                    "Specified to not load randbasis_A and randbasis_B from state dictionary however they are present in state"
                    " dictionary! Consider using them to ensure checkpoint loading is correct on all platforms using"
                    " `peft_config.save_projection = True`"
                )
            elif not config.save_projection:  # and no randbasis_A in state dictionary
                warnings.warn(
                    "Specified to not load randbasis_A and randbasis_B from state dictionary. This means we will be relying on"
                    " PRNG initialisation to restore these projections using `config.projection_prng_key`, which may"
                    " not be accurate on all system configurations."
                )
